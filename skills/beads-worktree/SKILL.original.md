...
This project uses bare git clones with worktrees. Beads lives in the main worktree. Other worktrees access it via `BEADS_DIR`.
...
All worktrees share ONE Beads database. Issues are centralized.
...
In each worktree shell session:
...
**Use BEADS_DIR.** It's the Beads-native solution.
...
Agents should always claim before working. If a task is already claimed, pick another.
